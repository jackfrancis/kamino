...
`vmss-prototype` is not getting the layout of the cluster
```
...
Note that running auto-update again, when there are no new updates available is a no-op and runs relatively quickly.  In are large production clusters we regularly (via a kubernetes cronjob) run the auto update process such that it builds a fresh prototype image automatically after OS patches are deployed and stable.
